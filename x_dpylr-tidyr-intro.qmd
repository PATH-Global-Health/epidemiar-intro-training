...
Notice here that just like in Chapter 1 use have to use a `==` sign for setting a condition. You read this as saying, "choose the rows in `case_data` where province is equal to"Eastern". Also notice that the number of rows in the object has gone down from `r nrow(case_data)` to `r nrow(filter(case_data, province == "Eastern"))`.
...
> (2) For each district in Eastern Province, calculate the test positivity rate at health facilities and by CHWs in 2020 (Test positivity = confirmed cases / total tests)
